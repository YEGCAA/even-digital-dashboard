...
async function inspect() {
    const { data, error } = await supabase.from('Vendas_2').select('status_venda_2').limit(1);
    if (error) {
        console.log('Explicit select lowercase error:', error.message);
    } else {
        console.log('Explicit select lowercase success:', data);
    }
}
