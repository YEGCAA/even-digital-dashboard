...
async function inspect() {
    const { data, error } = await supabase.from('Vendas_Pedrosa').select('*').limit(1);
    if (error) {
        console.log('Vendas_Pedrosa error:', error.message);
    } else {
        console.log('Vendas_Pedrosa success keys:', Object.keys(data[0] || {}));
    }
}
