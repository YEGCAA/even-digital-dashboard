...
async function inspect() {
    const { data, error } = await supabase.from('Vendas_3').select('*').limit(1);
    if (error) {
        console.log('Vendas_3 error:', error.message);
    } else {
        console.log('Vendas_3 success keys:', Object.keys(data[0] || {}));
    }
}
