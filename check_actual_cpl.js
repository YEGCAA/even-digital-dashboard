...
async function checkActualCP() {
    ...
    for (const table of tables) {
        ...
        if (data) {
            data.forEach(row => {
                const spendVal = row["Amount Spent"] || row["investimento"] || row["valor gasto"] || row["custo"] || row["gastos"];
                const leadsVal = row["Leads"] || row["lead count"] || row["leads_gerados"] || row["results"];

                if (spendVal) {
                    let s = spendVal.toString().replace(/[R$\sBRL]/g, '').trim();
                ...
                if (leadsVal) {
                    const num = parseInt(leadsVal);
                    if (!isNaN(num)) totalLeads += num;
                }
            });
        }
    }
    ...
    console.log('Actual CPL:', totalLeads > 0 ? totalSpend / totalLeads : 0);
}
