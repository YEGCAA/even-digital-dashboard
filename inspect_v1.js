...
async function inspectVendas1() {
    const { data, error } = await supabase.from('Vendas_1').select('*').limit(1);
    if (error) {
        console.log('Vendas_1 Error:', error.message);
    } else {
        console.log('Vendas_1 columns:', Object.keys(data[0] || {}));
    }
}
